...
OPTS=`getopt -o h --long mem:,cpu:,disk:,ip:,mask:,gw:,dns:,br:,vmname:,domain:,erp:,help -n 'parse-options' -- "$@"`
...
virt-install -d --name=$VM_NAME --ram $MEMORY --vcpus $CPU --disk vol=shared/$VM_NAME-disk0,bus=virtio,cache=none,format=raw --network bridge=$BRIDGE,model=virtio --vnc --vnclisten="0.0.0.0" --accelerate --location=http://mirror.yandex.ru/ubuntu/dists/trusty-updates/main/installer-amd64/ --extra-args="auto=true text file=file:/preseed.cfg passwd/root-password-crypted=$ENCRYPTED_ROOT_PASSWORD netcfg/get_ipaddress=$IPADDR netcfg/get_netmask=$NETMASK netcfg/get_gateway=$GATEWAY netcfg/get_nameservers=$DNS netcfg/disable_autoconfig=true netcfg/get_hostname=$VM_NAME netcfg/get_domain=$DOMAIN" --initrd-inject $TEMPFILE/preseed.cfg
